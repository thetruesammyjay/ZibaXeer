...
```
1. ArgusOracle
2. RiskManager        --constructor ArgusOracle
3. VaultRegistry
4. RevenueSplitter    --constructor treasury, leaderShareBps, protocolFeeBps
5. PaxDexAdapter      --constructor paxDexRouter
6. CopyTradingVault   (implementation only, no init)
7. VaultFactory       --constructor VaultRegistry, RiskManager, CopyTradingVault impl
```

After deployment:
- Call `VaultRegistry.setAuthorizedFactory(VaultFactory address)`
- Call `VaultFactory.initialize(owner, implAddress, registryAddress, riskManagerAddress)`
...
> ⚠️ Production upgrades should go through a **multi-sig** (e.g. Gnosis Safe) — never upgrade directly from a single EOA.
