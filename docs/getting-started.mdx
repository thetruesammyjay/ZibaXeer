...
<Steps>
  <Step title="Terminal 1 — Backend API">
    ```bash
    cd apps/backend
    pnpm dev
    ```
  ...
  <Step title="Terminal 3 — Frontend">
    ```bash
    cd apps/frontend
    pnpm dev
    ```
...
<AccordionGroup>
  <Accordion title="`pnpm install` fails on a package">
  ...
  <Accordion title="Indexer not picking up events">
